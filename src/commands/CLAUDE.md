...
- **task** – Blocks Task tool in Klaude sessions, redirects to `klaude start` with `--attach`/`--detach` options. Exempts `Plan` and `Explore` agents (allowed to use native Task tool)
